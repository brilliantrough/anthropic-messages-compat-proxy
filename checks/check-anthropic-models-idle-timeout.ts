...
async function main() {
  const primary = createServer(async (req, res) => {
    if (req.method === 'GET' && req.url === '/v1/models') {
      ...
      res.write('{"data":[');
      await delay(1000);
      res.end('{"id":"late-model","type":"model"}],"has_more":false}');
      return;
    }
  ...
  try {
    ...
    assert.ok(elapsed < 500, `models idle timeout should happen fast, got ${elapsed}ms`);

    const stdoutText = stdout.join('');
    assert.match(stdoutText, /forwarding models request/, 'expected models forwarding log');
    assert.match(stdoutText, /models response body timeout/, 'expected models idle body-timeout log');

    console.log('Anthropic models idle timeout check passed.');
  } finally {
    proxy.kill('SIGTERM');
    await Promise.race([
      once(proxy, 'exit'),
