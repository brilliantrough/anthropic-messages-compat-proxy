...
async function main() {
  ...
  const upstream = createServer(async (req, res) => {
    if (req.method === 'POST' && req.url === '/v1/responses') {
      ...
      capturedRequestBodies.push(JSON.parse(Buffer.concat(chunks).toString('utf8')) as JsonValue);
  ...
  try {
    ...
    const explicitReasoningResponse = await fetch(`http://127.0.0.1:${proxyPort}/v1/responses`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        model: 'mock-model',
    ...
    assert.equal(explicitReasoningResponse.status, 200);
    const explicitReasoningBody = capturedRequestBodies[1];
    assert.ok(explicitReasoningBody && typeof explicitReasoningBody === 'object' && !Array.isArray(explicitReasoningBody));
    assert.deepEqual((explicitReasoningBody as { reasoning?: unknown }).reasoning, { effort: 'low' });
